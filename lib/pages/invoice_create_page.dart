...
class _InvoiceCreatePageState extends State<InvoiceCreatePage> {
  ...
  final Map<String, int> _catalogStockByName = {};
  final Map<String, int> _catalogProductIdByName = {};
  ...
  double get discount =>
      double.tryParse(
        _discountController.text.replaceAll(RegExp(r'[٬،٫]'), '.'),
      ) ??
      0;
  ...
  Future<void> _selectProduct(
    ...
    final result = await Supabase.instance.client
        .from('products')
        .select('id,name,price,remaining_qty')
        .eq('user_id', user.id)
          ...
          child: products.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    final remainingQty =
                        product['remaining_qty']?.toString() ?? '0';
                    return ListTile(
                      title: Text(name),
                      subtitle: Text(
                        'السعر: ${price.toStringAsFixed(2)} • الكمية المتاحة: $remainingQty',
                      ),
                      enabled: _parseToDouble(product['remaining_qty']) > 0,
                      onTap: _parseToDouble(product['remaining_qty']) <= 0
                          ? null
                          : () {
                              descC.text = name;
                              priceC.text = price.toStringAsFixed(2);
                              _catalogStockByName[name] =
                                  int.tryParse(remainingQty) ?? 0;
                              final productId = int.tryParse(
                                product['id']?.toString() ?? '',
                              );
                              if (productId != null) {
                                _catalogProductIdByName[name] = productId;
                              }
  ...
  void _showItemDialog({InvoiceItemModel? item, int? index}) {
    ...
    final taxC = TextEditingController(
      text: item != null ? item.tax.toString() : '0',
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(
              controller: taxC,
              decoration: const InputDecoration(labelText: 'الضرائب'),
              keyboardType: TextInputType.numberWithOptions(decimal: true),
            ),
            TextField(
              controller: noteC,
  ...
  Future<void> _saveInvoice() async {
    ...
    for (final item in _items) {
      final available = _catalogStockByName[item.description];
      if (available != null && item.quantity > available) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'الكمية المطلوبة من "${item.description}" تتجاوز المتوفر ($available)',
            ),
          ),
        );
        return;
    ...
    final invoiceNumber =
        '${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(1000).toString().padLeft(3, '0')}';
    final invoice = {
    ...
    final items = _items
        .map((e) => _normalizeInvoiceItemMap(e.toMap()))
        .toList();
    try {
      await InvoiceDatabase.instance.insertInvoice(invoice, items);
      for (final item in _items) {
        final available = _catalogStockByName[item.description];
        final productId = _catalogProductIdByName[item.description];
        if (available == null || productId == null) continue;
        await Supabase.instance.client
            .from('products')
            .update({'remaining_qty': available - item.quantity.toInt()})
            .eq('id', productId);
      }
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('خطأ عند الحفظ: $e')));
    } finally {
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText: 'اكتب ملاحظات عامة تظهر في الفاتورة',
                ),
                maxLines: 3,
              ),
              const SizedBox(height: 12),
              Row(
                children: [
                  Expanded(
                    child: Text('تاريخ الفاتورة: ${dateFmt.format(_date)}'),
                  ),
                  TextButton(
                    onPressed: () async {
                      final picked = await showDatePicker(
              ...
              Row(
                children: [
                  Expanded(
                    child: Text(
                      'تاريخ الاستحقاق: ${_dueDate != null ? dateFmt.format(_dueDate!) : '-'}',
                    ),
                  ),
                  TextButton(
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: () =>
                                            _showItemDialog(item: it, index: i),
                                      ),
                                      IconButton(
                                        icon: const Icon(
                                          Icons.delete,
